...
uint8_t U30HOST_MS_CofDescrAnalyse( uint8_t *pbuf,uint8_t port )
{
	...
	if( (SS_HUB_SaveData.Device_Type == HUB_DEVICE) && (SS_HUB_SaveData.Depth == 0)){			
		if( ( ( (PUSB_CFG_DESCR_LONG_U30)pbuf ) -> endp_descr[ 0 ].bLength == 0x07)
			...
			&& ( (((PUSB_CFG_DESCR_LONG_U30)pbuf ) -> endp_descr[ 0 ].bmAttributes&0x0f) == 0x03) ){		
				SS_HUB_SaveData.HUB_Info.IntEndpnum = ( (PUSB_CFG_DESCR_LONG_U30)pbuf ) -> endp_descr[ 0 ].bEndpointAddress & 0x0F;
...
uint8_t U20HOST_MS_CofDescrAnalyse( uint8_t *pbuf ,uint8_t port)
{
	...
	gDiskBulkInEp  = 0;
	gDiskBulkOutEp = 0;
	if( (HS_HUB_SaveData.Device_Type == HUB_DEVICE) && (HS_HUB_SaveData.Depth == 0)){			
		...
		if( ( ( (PUSB_CFG_DESCR_LONG)pbuf ) -> endp_descr[ 0 ].bLength == 0x07)
			...
			&& ( (((PUSB_CFG_DESCR_LONG)pbuf ) -> endp_descr[ 0 ].bmAttributes&0x0f) == 0x03) ){		
			HS_HUB_SaveData.HUB_Info.IntEndpnum = ( (PUSB_CFG_DESCR_LONG)pbuf ) -> endp_descr[ 0 ].bEndpointAddress & 0x0F;
