...
//#define DISK_BASE_BUF_LEN     512 /* Default disk data buffer size is 512 bytes(can be set to 2048 or even 4096 to support U‑disks with large sectors), set to 0 to disable buffer definition in this file and assign buffer start address via pDISK_BASE_BUF in application */
/* To save RAM by reusing the disk data buffer, define DISK_BASE_BUF_LEN as 0 to skip buffer allocation here. Assign the shared buffer address to pDISK_BASE_BUF before calling CHRV3LibInit in application code */
//#define NO_DEFAULT_ACCESS_SECTOR      1       /* Disable default disk sector read/write routine, replace it with self‑implemented function below */
//#define NO_DEFAULT_DISK_CONNECT       1       /* Disable default disk connection check routine, replace it with self‑implemented function below */
//#define NO_DEFAULT_FILE_ENUMER        1       /* Disable default filename enumeration callback, replace it with self‑implemented function below */
//#include "CHRV3SFR.H"
...
{
    (void)endp_pid, (void)tog, (void)timeout;
...
#ifndef NO_DEFAULT_ACCESS_SECTOR        /* Define NO_DEFAULT_ACCESS_SECTOR in application to disable the default disk read/write routine and replace it with a custom implementation */
//if ( use_external_interface ) {  // Replace low‑level U‑disk sector read/write routine
//    CHRV3vSectorSize=512;  // Set actual physical sector size, must be multiple of 512 bytes
//    CHRV3vSectorSizeB=9;   // Set bit shift value for sector size, 9 for 512,10 for 1024,11 for 2048
//    CHRV3DiskStatus=DISK_MOUNTED;  // Force block device connected state (file system parsing pending)
//}
uint8_t CHRV3ReadSector( uint8_t SectCount, uint8_t *DataBuf )  /* Read multiple sectors from disk into buffer */
{
    uint8_t retry;
...
//  if ( use_external_interface ) return( extReadSector( CHRV3vLbaCurrent, SectCount, DataBuf ) );  /* External interface */
    ...
    for( retry = 0; retry < 3; retry ++ ) {  /* Retry on error */
        pCBW -> mCBW_DataLen = (uint32_t)SectCount << CHRV3vSectorSizeB;  /* Data transfer length */
        pCBW -> mCBW_Flag = 0x80;
...
//  if ( use_external_interface ) return( extWriteSector( CHRV3vLbaCurrent, SectCount, DataBuf ) );  /* External interface */
    for( retry = 0; retry < 3; retry ++ ) {  /* Retry on error */
        ...
        CHRV3BulkOnlyCmd( DataBuf );  /* Execute Bulk‑Only protocol command */
        if ( CHRV3IntStatus == ERR_SUCCESS ) {
            Delay_Us( 200 );  /* Delay after write operation */
            return( ERR_SUCCESS );
        }
        CHRV3IntStatus = CHRV3AnalyzeError( retry );
        if ( CHRV3IntStatus != ERR_SUCCESS ) {
...
#ifndef NO_DEFAULT_DISK_CONNECT         /* Define NO_DEFAULT_DISK_CONNECT in application to disable default disk detection routine and replace it with custom code */
/*
Agreement: USB device address allocation rule(refer to USB_DEVICE_ADDR)
...
uint8_t CHRV3DiskConnect( void )
{
    ...
    {
        /* USB device attached to built‑in Root‑HUB */
        if ( ums & bUMS_CONNECT )
        {
            /* USB device under built‑in Root‑HUB exists */
            if ( ( ums & bUMS_SUSPEND ) == 0 )
            {
                /* USB device exists and no hot‑plug event detected */
                return( ERR_SUCCESS );              /* USB device is connected without re‑plugging */
            }
            else
            {
                /* USB device under built‑in Root‑HUB exists */
                CHRV3DiskStatus = DISK_CONNECT;     /* Device was disconnected before */
                return( ERR_SUCCESS );              /* External HUB or USB device reconnected after disconnect */
            }
        }
        else
        {
...
#ifndef NO_DEFAULT_FILE_ENUMER          /* Define NO_DEFAULT_FILE_ENUMER in application to disable default filename enumeration callback and use self‑defined function */
void xFileNameEnumer( void )            /* Filename enumeration callback routine */
{
/* If CHRV3vFileSize is set to 0xFFFFFFFF before calling FileOpen, FileOpen will invoke this callback every time a file entry is found.
After returning from xFileNameEnumer, FileOpen decreases CHRV3vFileSize and continues enumeration until no more entries are found or directory scan completes.
Recommended usage: declare a global counter variable before FileOpen. Inside this callback, retrieve the FAT_DIR_INFO structure via CHRV3vFdtOffset,
parse DIR_Attr and DIR_Name to match target file/directory, record required information and increment the counter.
When FileOpen returns, treat ERR_MISS_FILE or ERR_FOUND_NAME as successful enumeration, the global counter holds the total matched files.
Set CHRV3vFileSize = 1 inside this callback to terminate the file search early. Example implementation is shown below */
#if     0
    uint8_t         i;
    uint16_t        FileCount;
    ...
    FileCount = (UINT16)( 0xFFFFFFFF - CHRV3vFileSize );  /* Current enumeration index, CHRV3vFileSize initialized to 0xFFFFFFFF and decremented on each match */
    if ( FileCount < sizeof( FILE_DATA_BUF ) / 12 ) {  /* Verify buffer capacity, assume 12 bytes per filename entry */
        NameBuf = & FILE_DATA_BUF[ FileCount * 12 ];  /* Calculate target buffer address for current filename */
        for ( i = 0; i < 11; i ++ ) NameBuf[ i ] = pFileDir -> DIR_Name[ i ];  /* Copy filename (11‑character raw entry, spaces not trimmed) */
//      if ( pFileDir -> DIR_Attr & ATTR_DIRECTORY ) NameBuf[ i ] = 1;  /* Mark entry as directory */
        NameBuf[ i ] = 0;  /* String terminator for filename */
    }
#endif
}
...
uint8_t CHRV3LibInit( void )  /* Initialize CHRV3 library, return 0 on success */
{
    ...
    if( s < CHRV3_LIB_VER )
    {
        return( 0xFF );  /* Retrieve library version number, return error if version is too old */
    }
    printf( "lib vision:%02x\r\n",s );
#if     DISK_BASE_BUF_LEN > 0
    pDISK_BASE_BUF = & DISK_BASE_BUF[0]; /* Point to disk data buffer in external RAM */
    pDISK_FAT_BUF = & DISK_BASE_BUF[0];  /* Point to FAT table buffer in external RAM, share buffer with pDISK_BASE_BUF for RAM saving */
//  pDISK_FAT_BUF = & DISK_FAT_BUF[0];   /* Point to independent FAT buffer separated from pDISK_BASE_BUF for higher access speed */
/* For better file access performance, reassign pDISK_FAT_BUF to another separate buffer with identical size after CHRV3LibInit in main program */
#endif
    CHRV3DiskStatus = DISK_UNKNOWN;     /* Unknown initial state */
    CHRV3vSectorSizeB = 9;              /* Default physical disk sector size: 512 bytes */
    CHRV3vSectorSize = 512;             /* Default physical disk sector size: 512 bytes, actual sector size of storage device */
    CHRV3vStartLba = 0;                 /* Auto‑detect FDD and HDD partition by default */
    CHRV3vPacketSize = 512;             /* Max packet length for USB mass storage:64@FS,512@HS/SS, initialized by application and updated after U‑disk enumeration for high‑speed/super‑speed devices */
