...
__attribute__ ((aligned(4))) uint8_t USBSS_EP1_Tx_Buf[ DEF_USB_EP1_SS_SIZE * 32 ];
...
void USBSS_IRQHandler( void )              
{
    ...
    if(( status & USBSS_UDIF_SETUP ) && !( status & USBSS_UDIF_STATUS ))    // SETUP-DPH & ACK-TP
    {
        ...
        if(( USBSS_SetupReqType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )
        {
            ...
            {
                errflag = USBSS_UVC_NonStandardReq( &pUSBSS_Descr );
                if(errflag != 0xFF)
                {
            ...
            {
                /*set cur*/
                if(( USBSS_SetupReqType == ( USB_REQ_TYP_CLASS | USB_REQ_RECIP_INTERF )) && ( USBSS_SetupReqCode == 0x01 ))
                {
                    len = 0;
                }
                else
        ...
        else 
        {
            switch( USBSS_SetupReqCode )
            {
                ...
                case USB_SET_INTERFACE:
                    USBSSD->EP1_TX.UEP_TX_CR |= USBSS_EP_TX_CHAIN_CLR;
                    SS_CtrlCamera();
                    break;
        ...
        else 
        {
            switch( endp_num )
            {
                ...
                case DEF_UEP0:
                    ...
                    if( rx_len == DEF_UVC10_VEDIO_CTL )
                    {
                        if( (format != *(UINT8 *)(USBSS_EP0_Buf + 2)) || (frame != *(UINT8 *)(USBSS_EP0_Buf + 3)))
                        {
                            format = *(UINT8 *)(USBSS_EP0_Buf + 2);
                            frame = *(UINT8 *)(USBSS_EP0_Buf+3);
                            Get_Curr[ 2 ] = format;
                            Get_Curr[ 3 ] = frame;
                            Formatchange_flag = *(UINT8 *)(USBSS_EP0_Buf + 2);
