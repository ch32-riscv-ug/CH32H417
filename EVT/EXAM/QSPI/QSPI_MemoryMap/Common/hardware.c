...
static void QSPI1_Config(void)
{
    ...
    QSPI_InitStructure.QSPI_Prescaler = 1;
 ...
 * @param   src - the src
 *          dst - the dst
 *          len - the len to copy. The value must be divisible by 4.
 * 
 * @return  none
 */
void Mem_Copy_DMA(uint8_t* src, uint8_t* dst, uint32_t len)
{
    DMA_InitTypeDef DMA_InitStructure = {0};

    DMA_DeInit(DMA1_Channel2);
    DMA_InitStructure.DMA_PeripheralBaseAddr = (u32)dst;
    DMA_InitStructure.DMA_Memory0BaseAddr    = (u32)src;
    DMA_InitStructure.DMA_DIR                = DMA_DIR_PeripheralDST;
    DMA_InitStructure.DMA_BufferSize         = len / 4;
 ...
 *          len - the length of the data to be programmed. The value must be divisible by 256.
 * 
 * @return  none
 */
 ...
 *          len - The length of the written data.It must be a multiple of 256
 * 
 * @return  none
 */
...
void Hardware(void)
{
    ...
    uint8_t* QSPI_MapAddr = (void*)0x90000000;
    uint8_t  readData[64];
    ...
    uint8_t*  QSPI_MapAddr8  = (void*)0x90000000;
    uint16_t* QSPI_MapAddr16 = (void*)0x90000000;
    uint32_t* QSPI_MapAddr32 = (void*)0x90000000;

    for (size_t i = 0; i < 10; i++)
    {
        printf("u8 [%x] = %x\n", &(QSPI_MapAddr8[i]), QSPI_MapAddr8[i]);
        printf("u16[%x] = %x\n", &(QSPI_MapAddr16[i]), QSPI_MapAddr16[i]);
        printf("u32[%x] = %x\n", &(QSPI_MapAddr32[i]), QSPI_MapAddr32[i]);
    }
