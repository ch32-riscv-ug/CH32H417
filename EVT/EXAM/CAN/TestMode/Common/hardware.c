 ...
 * @note   CAN1(+0): PA12, PB7, PB9, PD1, PA14
 *         CAN2(+5): PB13, PB6
 *         CAN3(+7): PD13, PF7, PF3, PC5
 */
 ...
 * @note   CAN1(+0): PA11, PB6, PB8, PD0, PA13
 *         CAN2(+5): PB12, PB5
 *         CAN3(+7): PD12, PF6, PF4, PC4
 */
...
//	CAN_FilterInitSturcture.CAN_FilterMaskIdLow = 0;
    ...
    CAN_Mode_Init(CAN2, CANRXPinTable + 5, CANTXPinTable + 5, &CAN_InitSturcture);
    CAN_Mode_Init(CAN3, CANRXPinTable + 7, CANTXPinTable + 7, &CAN_InitSturcture);
    ...
    CAN_FilterInitSturcture.CAN_FilterNumber = 15;
    CAN_SoftFilterInit(&CAN_FilterInitSturcture);
    CAN_FilterInitSturcture.CAN_FilterNumber = 30;
    CAN_SoftFilterInit(&CAN_FilterInitSturcture);
#else
...
__attribute__((interrupt("WCH-Interrupt-fast"))) void CAN2_RX0_IRQHandler()
{
    int i;
    uint8_t px, pbuf[8];
    if (CAN_GetITStatus(CAN2, CAN_IT_FMP0))
    {
        px = CAN_Receive_Msg(CAN2, pbuf);
        for (i = 0; i < px; i++)
        {
            RxBuff[i] = pbuf[i];
        }
...
__attribute__((interrupt("WCH-Interrupt-fast"))) void CAN3_RX0_IRQHandler()
{
    int i;
    uint8_t px, pbuf[8];
    if (CAN_GetITStatus(CAN3, CAN_IT_FMP0))
    {
        px = CAN_Receive_Msg(CAN3, pbuf);
        for (i = 0; i < px; i++)
        {
            RxBuff[i] = pbuf[i];
        }
        if (px)
        {
            RxInterruptFlag[2] = px;
        }
        CAN_ClearITPendingBit(CAN3, CAN_IT_FMP0);
    }
}
#endif
