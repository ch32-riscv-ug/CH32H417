...
SD_Error eMMC_Init( void )
{
    ...
    RCC_HB2PeriphClockCmd(RCC_HB2Periph_SDIO|RCC_HBPeriph_DMA1, ENABLE);
    //CLK  CMD
    ...
    GPIO_Initure.GPIO_Pin=GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_10|GPIO_Pin_11|GPIO_Pin_12|GPIO_Pin_13|GPIO_Pin_14|GPIO_Pin_15; 
    GPIO_Initure.GPIO_Mode=GPIO_Mode_AF_PP;
    GPIO_Initure.GPIO_Speed=GPIO_Speed_Very_High;
    ...
    GPIO_PinAFConfig(GPIOE,GPIO_PinSource12,GPIO_AF8 );
    GPIO_PinAFConfig(GPIOE,GPIO_PinSource13,GPIO_AF8 );
    GPIO_PinAFConfig(GPIOE,GPIO_PinSource14,GPIO_AF8 );
    GPIO_PinAFConfig(GPIOE,GPIO_PinSource15,GPIO_AF8 );
    ...
    if( ( errorstatus == SD_OK ) || ( SDIO_MULTIMEDIA_CARD == CardType ) )
    {
        ...
        // SDIO_Clock_Set( clkdiv ); //CLKDIV mode:SDIO_CLK=HCLK/(clkdiv+2)
        SDIO->CLKCR|=(1<<10);   //BYPASS mode:SDIO_clk=Hclk
         SDIO->CLKCR|=(1<<14);   
 ...
 * @fn      eMMC_ChangeHS200
 *
 * @brief   eMMC eMMC_Change HS200 mode
 *
 * @return  SD_Error - ERR code
 */
SD_Error eMMC_ChangeHS200( void )
{
    SD_Error errorstatus = SD_OK;
    SDIO_CmdInitStructure.SDIO_Argument = 0x3B90200;
...
SD_Error SD_ReadBlock( u8 *buf, long long addr, u16 blksize )
{
    ...
    else if( DeviceMode == SD_DMA_MODE )
    {
       SD_DMA_Config( ( u32 * )buf, blksize, DMA_DIR_PeripheralSRC );
        ...
        SDIO_DMACmd( ENABLE );
        while( ( ( DMA1->INTFR & 0X2 ) == RESET ) && ( TransferEnd == 0 ) && ( TransferError == SD_OK ) && timeout )
        {
            timeout--;
        }
        if( timeout == 0 )
...
SD_Error SD_ReadMultiBlocks( u8 *buf, long long addr, u16 blksize, u32 nblks )
{
    ...
    if( nblks > 1 )
    {
        ...
        else if( DeviceMode == SD_DMA_MODE )
        {
            ...
            SDIO->MASK |= ( 1 << 1 ) | ( 1 << 3 ) | ( 1 << 8 ) | ( 1 << 5 ) | ( 1 << 9 );
            SDIO->DCTRL |= 1 << 3;
            while( ( ( DMA1->INTFR & 0X2 ) == RESET ) && timeout )
            {
                timeout--;
            }
            if( timeout == 0 )
...
void SD_DMA_Config( u32 *mbuf, u32 bufsize, u32 DMA_DIR )
{
    ...
    DMA_MuxChannelConfig(DMA_MuxChannel1,111);
