...
#define SDMMC_DMADDR_ALIGN_SIZE            32
uint8_t     *SDIO_Write_Buf;
uint8_t     *SDIO_Read_Buf;
uint32_t    SDIO_Write_Buf_Len;
uint32_t    SDIO_Read_Buf_Len;
uint8_t     SDMMC_ErrorTimes = 0x00;                                          
 ...
 * @return  SD_Error - ERR code
 */
/* access : 0 - change CMD set; 1 - set bits; 2 - clear bits; 3 - write byte */
...
SD_Error eMMC_Init()
{
    ...
    if( errorstatus == SD_OK )
    { 
        errorstatus = MMC_AccessEXTCSD_Polling( 3, 185,2);   
    }
