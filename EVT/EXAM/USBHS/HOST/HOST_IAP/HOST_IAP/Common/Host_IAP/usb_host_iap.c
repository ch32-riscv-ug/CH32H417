...
uint8_t IAP_Flash_Read(uint32_t address, uint8_t* buff, uint32_t length)
{
    ...
    if(((read_addr >= DEF_APP_CODE_START_ADDR) && ((read_addr + length) <= DEF_APP_CODE_END_ADDR)) 
    || ((read_addr >= DEF_VERIFY_CODE_START_ADDR) && ((read_addr + length) <= DEF_VERIFY_CODE_END_ADDR)))
    {
        /* Available Data */
...
uint8_t IAP_Flash_Write(uint32_t address, uint8_t* buff, uint32_t length)
{
    ...
    uint32_t full_page_len,last_page_len,last_page_address;
    uint8_t s;
    /* Verify Keys, No flash operation if keys are not correct */
    if((Flash_Operation_Key0 != DEF_FLASH_OPERATION_KEY_CODE_0) || (Flash_Operation_Key1 != DEF_FLASH_OPERATION_KEY_CODE_1))
    {
    ...
    /* Verify Address, No flash operation if the address is out of range */
    if(((address >= DEF_APP_CODE_START_ADDR) && ((address + length) <= DEF_APP_CODE_END_ADDR)) 
    || ((address >= DEF_VERIFY_CODE_START_ADDR) && ((address + length) <= DEF_VERIFY_CODE_END_ADDR)))
    {
        full_page_len = (length / DEF_FLASH_PAGE_SIZE) * DEF_FLASH_PAGE_SIZE;
        last_page_len = length % DEF_FLASH_PAGE_SIZE;
        if(full_page_len)
        {
            FLASH_ROM_WRITE(address,(uint32_t*)buff,full_page_len);
        }

        if(last_page_len)
        {
            last_page_address = address + full_page_len;
            IAP_Flash_Read(last_page_address, temp_buf, DEF_FLASH_PAGE_SIZE);
            memcpy(temp_buf, buff + full_page_len, last_page_len);
            s = FLASH_ROM_WRITE(last_page_address, (uint32_t *)temp_buf, DEF_FLASH_PAGE_SIZE);
            if(s != FLASH_COMPLETE)
            {
                return s;
            }
 ...
 * @fn      IAP_Flash_Erase
 *
 * @brief   Erase Flash In Block(8k bytes),Specify length & address,
 *          Based On Fast Flash Operation,
 *          With address protection and program runaway protection.
 * @param   length - erase length must be aligned to 8K.
 * @return  0: Operation Success
 *          See notes for other errors
 */
uint8_t IAP_Flash_Erase(uint32_t address, uint32_t length)
{
    ...
    || ((address >= DEF_VERIFY_CODE_START_ADDR) && ((address + length) <= DEF_VERIFY_CODE_END_ADDR)))
    {
        FLASH_ROM_ERASE(address,length);
    }
    else
    {
        /* ERR: address error */
...
uint32_t IAP_Flash_Verify(uint32_t address, uint8_t* buff, uint32_t length)
{
    ...
    pcode = (uint8_t*)(uint32_t)address;
...
void IAP_Main_Deal( void )
{
    ...
    if( (CHRV3DiskStatus >= DISK_MOUNTED) && op_flag )
    {
        ...
        else
        {
            ...
            /* Make sure the flash operation is correct */
            Flash_Operation_Key0 = DEF_FLASH_OPERATION_KEY_CODE_0;
            DUG_PRINTF( "Erase App\r\n");
            IAP_Flash_Erase(DEF_APP_CODE_START_ADDR, DEF_APP_CODE_MAXLEN);
            IAP_Load_Addr_Offset = 0;
            IAP_WriteIn_Length = 0;
            IAP_WriteIn_Count = 0;
