...
void Refresh_Basic(u8 *buf)
{
    ...
    WEB_ERASE( BASIC_CFG_ADDR, FLASH_ERASE_SIZE);
...
void Refresh_Port(char *buf)
{
    ...
    WEB_ERASE( PORT_CFG_ADDR, FLASH_ERASE_SIZE);
...
void Refresh_Login(char *buf)
{
    ...
    WEB_ERASE( LOGIN_CFG_ADDR, FLASH_ERASE_SIZE);
...
void WEB_ERASE(uint32_t Page_Address, u32 Length) {
    ...
    FLASH_Unlock_Fast();
    NbrOfPage = Length / FLASH_ERASE_SIZE;

    for (EraseCounter = 0; EraseCounter < NbrOfPage; EraseCounter++) {
        FLASH_ErasePage( Page_Address + (FLASH_ERASE_SIZE * EraseCounter)); //Erase 8kB
    }
    FLASH_Lock_Fast();
}
