...
int32_t FLT0_Rxdata[RxBuffSize] = {0};
int32_t FLT1_Rxdata[RxBuffSize] = {0};
...
void DFSDM_Function_Init(void)
{
    ...
    DFSDM_ChannelInitStructure.DFSDM_ChDataRightBitShift = 4;
    ...
    /* initialize DFSDM regular conversions */
...
void Hardware(void)
{
	...
	Internal_Data();
    DMA_Rx_Init(DMA1_Channel1, (int32_t)& DFSDM_FLT0->RDATAR, (int32_t)FLT0_Rxdata, RxBuffSize);
    DMA_Rx_Init(DMA1_Channel2, (int32_t)& DFSDM_FLT1->RDATAR, (int32_t)FLT1_Rxdata, RxBuffSize);
    ...
    for(i = 0; i < TxbuffSize; i++)
    {
        ...
        /* Write the parallel data on standard mode of data packing. */
        DFSDM_WriteParallelDataStanMode(DFSDM_Channel1, Txdata[i]);
    }
    ...
    {
        printf("FLT0_Rxdata:%d   FLT1_Rxdata:%d\r\n", (FLT0_Rxdata[i] >> 8), (FLT1_Rxdata[i] >> 8));
    }	
