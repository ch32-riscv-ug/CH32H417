...
{                                            
    ...
    memcpy(USBFSD_UEP_BUF(DEF_UEP2), pEndp2_Buf, len);
...
void UDISK_Up_CSW( void )
{
    ...
    /* Load the data into the upload buffer and start the upload */
    memcpy(USBFSD_UEP_BUF(DEF_UEP2),(uint8_t *)mBOC.buf,0x0D);
    USBFSD->UEP2_TX_LEN = 0x0D;
    USBFSD->UEP2_TX_CTRL = (USBFSD->UEP2_TX_CTRL & ~USBFS_UEP_T_RES_MASK) | USBFS_UEP_T_RES_ACK;
}
...
{    
    ...
    IFlash_Read(UDISK_Cur_Sec_Lba * DEF_UDISK_SECTOR_SIZE + UDISK_Pack_Size * UDISK_Sec_Pack_Count, 
                UDISK_Pack_Size, 
                UDisk_Pack_Buffer);
#endif
	pbuf = UDisk_Pack_Buffer;
    /* USB upload this package data */
    memcpy(USBFSD_UEP_BUF(DEF_UEP2),pbuf,UDISK_Pack_Size);
    USBFSD->UEP2_TX_LEN = UDISK_Pack_Size;
    USBFSD->UEP2_TX_CTRL = (USBFSD->UEP2_TX_CTRL & ~USBFS_UEP_T_RES_MASK) | USBFS_UEP_T_RES_ACK;
...
#elif (STORAGE_MEDIUM == MEDIUM_INTERAL_FLASH)
        IFlash_Prog_4K(sec_start_addr,(uint32_t*)UDisk_Down_Buffer);
#endif
        if( UDISK_Transfer_DataLen == 0x00 )
        {
            Udisk_Transfer_Status &= ~DEF_UDISK_BLUCK_DOWN_FLAG;
