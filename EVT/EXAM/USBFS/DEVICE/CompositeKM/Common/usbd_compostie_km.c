...
void TIM3_Init( uint16_t arr, uint16_t psc )
{
    ...
    NVIC_SetPriority(TIM3_IRQn,0x80);                                           // Preemption Priority:1
...
void USART4_Receive_Handle( void )
{
    ...
    {
        if((USBFSD->UEP1_TX_CTRL & USBFS_UEP_T_RES_MASK) == USBFS_UEP_T_RES_NAK)
        {
            memcpy(USBFSD_UEP_BUF(DEF_UEP1), KB_Data_Pack, sizeof(KB_Data_Pack));
            USBFSD->UEP1_TX_LEN = sizeof(KB_Data_Pack);
...
void KB_Sleep_Wakeup_Cfg( void )
{
    ...
    __WFE_IDSET(EXTI_Line11 | EXTI_Line12 | EXTI_Line13 | EXTI_Line14);
}
...
void MS_Sleep_Wakeup_Cfg( void )
{
    ...
    __WFE_IDSET(EXTI_Line4 | EXTI_Line5 | EXTI_Line6 | EXTI_Line7);
}
...
void MS_Scan_Handle( void )
{
    ...
    if( flag )
    {
        ...
        {
            memcpy(USBFSD_UEP_BUF(DEF_UEP2), MS_Data_Pack, sizeof(MS_Data_Pack));
            USBFSD->UEP2_TX_LEN = sizeof(MS_Data_Pack);
...
void MCU_Sleep_Wakeup_Operate( void )
{
    ...
    EXTI_ClearFlag( EXTI_Line11 | EXTI_Line12 | EXTI_Line13 | EXTI_Line14 );
    EXTI_ClearFlag( EXTI_Line4 | EXTI_Line5 | EXTI_Line6 | EXTI_Line7 );

    __WFE();

    wakeup_source = EXTI_GetWFEWkupSource();
    SystemInit();
    USBFS_RCC_Init();

    if( wakeup_source & ( EXTI_Line11 | EXTI_Line12 | EXTI_Line13 | EXTI_Line14 ))
    {
        EXTI_ClearFlag(EXTI_Line11 | EXTI_Line12 | EXTI_Line13 | EXTI_Line14 );
        USBFS_Send_Resume( );
    }
    else if(wakeup_source & ( EXTI_Line4 | EXTI_Line5 | EXTI_Line6 | EXTI_Line7) )
    {
        EXTI_ClearFlag( EXTI_Line4 | EXTI_Line5 | EXTI_Line6 | EXTI_Line7 );
        USBFS_Send_Resume( );
    }
