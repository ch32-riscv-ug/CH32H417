...
static void RxInit(void)
{
    ...
    NVIC_SetPriority(EXTI7_0_IRQn,0<<4);
    ...
    TIM_TimeInitStructure.TIM_Prescaler = (HCLKClock / (SoftUARTPara.baud_rate * 10) - 1);
...
{
    GPIO_InitTypeDef GPIO_InitStructure={0};
    GPIO_PinAFConfig(SoftUARTHardware.txPort, GPIO_PinSource3, GPIO_AF0);
    ...
    DMA_InitStruct.DMA_Memory0BaseAddr = (u32)NULL;
...
static void TransmitBytes(vu8 *data, vu16 len)
{
    ...
    if (buffer != NULL)
    {
        for (u16 i = 0; i < (len > 0 ? len : length); i++)
        {
            ...
            DMA_Cmd(SoftUARTHardware.txDMA, DISABLE);
        }
    }
    free(buffer);
}
...
void Init(u32 baudrate)
{
    ...
    SoftUARTHardware.rxPort = GPIOE;
    SoftUARTHardware.rxPin = GPIO_Pin_4;
    SoftUARTHardware.txTIM = TIM8;
    SoftUARTHardware.txPort = GPIOE;
    SoftUARTHardware.txPin = GPIO_Pin_3;
